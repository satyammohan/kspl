...
*/
    ...
    function prsale($id = "") {
        ...
        $this->sm->assign("sale", $res1);
        $sql = "SELECT s.*, b.expiry_date, b.mrp_without_tax, b.distributor_price, p.name AS item, p.hsncode, p.case, p.pack, p.unit FROM {$this->prefix}saledetail s LEFT JOIN {$this->prefix}batch b ON s.id_batch=b.id_batch, {$this->prefix}product p WHERE s.id_product=p.id_product AND s.id_sale IN ($id)";
        $res = $this->m->sql_getall($sql, 1, "", "id_sale", "id_saledetail");
        $this->sm->assign("saledetail", $res);
        $this->sm->assign("discount", $this->ini['discount']);
        $sql = "SELECT id_taxmaster AS id,name FROM {$this->prefix}taxmaster";
