    ...
    function register() {
        ...
        switch ($_REQUEST['option']) {
            case 1:
                ...
                $sql = "SELECT s.* FROM `{$this->prefix}partner_sale` s WHERE `date` >= '$sdate' AND `date` <= '$edate' {$wcond} ORDER BY date";
                break;
            case 2:
            ...
            case 3:
                $sql = "SELECT MONTHNAME(`date`) AS month, MONTH(`date`) AS m, YEAR(`date`) AS year, 
                    SUM(IF(!h.local, 0, 1)*gst) AS igst, SUM(IF(!h.local, 1, 0)*(gst/2)) AS cgst, SUM(IF(!h.local, 1, 0)*(gst/2)) AS sgst,
                    SUM(gst) AS vat, SUM(`total`) AS `total`, SUM(`totalamt`-s.discount) AS `totalamt`,
