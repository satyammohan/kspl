...
//$this->pr($ledger);
    ...
    function outstandingc()  {
        ...
        $sql = "SELECT h.id_head, p.total FROM `{$this->prefix}head` h, (SELECT id_head, SUM(t.total1) AS total FROM (
                ...
                GROUP BY 1) p WHERE h.id_head=p.id_head AND h.creditor AND p.total<>0
                GROUP BY h.id_head  ORDER BY h.id_head ";
        $res = $this->m->sql_getall($sql, 2, "total", "id_head");
        $wcond = " ";
        if (@$_REQUEST['area']) {
        ...
        $sql = "SELECT h.name, '' AS date, r.id_represent, a.name AS aname, 0 AS id_company, r.name AS rname, '' AS cname, h.id_area, 'OPENING' AS invno, h.id_head,
